...
char *welcome(char* hello, char* name) {
    char *message = (char *)malloc(sizeof(char) * (strlen(hello) + strlen(name) + 3));
    if (message == NULL) {
...
int quartile(char* name) {
    char c = tolower(name[0]); // Convert the first character to lowercase for case-insensitive comparison
    ...
    if (c >= 'a' && c <= 'f') {
        ...
        return -1; // Handle names outside these ranges.
    }
}
